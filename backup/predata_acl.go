...
func PrintExternalGrants(file *utils.FileWithByteCount, toc *toc.TOC,
	metadata ObjectMetadata, info FunctionInfo) {
	_, entry := metadata.GetMetadataEntry()
	if entry.ObjectType == "FUNCTION" || entry.ObjectType == "AGGREGATE" {
		statements := make([]string, 0)
		fqn := fmt.Sprintf("%s.%s(%s)", entry.Schema, entry.Name, info.IdentArgs)
		if privileges := metadata.GetPrivilegesStatements(fqn, "FUNCTION"); privileges != "" {
			statements = append(statements, strings.TrimSpace(privileges))
		}
		PrintStatements(file, toc, metadata, statements)
	}
}
